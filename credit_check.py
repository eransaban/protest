...
def check_credit(arg):
    ...
    if arg[0] != "4" and arg[0] != "5" and arg[0] != "6": #check if number start with 4,5,6
        id = False
    ...
    for i in arg:  #check if there's space in the number (not included in the symbols list)
        if i == " ":
            id = False
            raise SymbolsUsed
    for i in range(len(arg)-3): # check for consecutive and repeated numbers
        if arg[i] == arg[i+1]:
            if arg[i+1] == arg[i + 2]:
                if arg[i + 2] == arg[i + 3]:
                    id = False
...
def main():
    """ Ask for a credit card number and runs check
    first of all strip hypens and then run test on the remaining digits and raise appropriate error """
    global credit
    creditcard = input("Hi Please enter your Credit Card Number:")
    try:
